...
import static io.apicurio.studio.operator.utils.TraverseUtils.*;
        ...
        where(spec.getSpec().getContainers(), sc -> containerName.equals(sc.getName()), sc -> {
            where(base.getSpec().getContainers(), fc -> containerName.equals(fc.getName()), fc -> {
                ...
                if (!isEmpty(sc.getEnv())) {
                    log.warn(".spec.containers[name = {}].env field cannot be used updated by PodTemplateSpec", containerName); // TODO: Status condition
                }
                sc.setEnv(List.of()); // TODO: This might not even be necessary as it will be overridden

                if (!isEmpty(sc.getImage())) {
                    log.warn(".spec.containers[name = {}].image field cannot be used updated by PodTemplateSpec", containerName); // TODO: Status condition
                }

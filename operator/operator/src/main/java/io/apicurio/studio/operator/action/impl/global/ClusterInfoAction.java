package io.apicurio.studio.operator.action.impl.global;
        ...
        if (clusterInfo.getCanonicalHost() == null) {
            clusterInfo.getIsOCP().ifPresent(isOCP -> {
                if (isOCP) {
                    ...
                    var routes = oc.routes()
                            .inAnyNamespace()
                            .withLabel("app.kubernetes.io/name", "apicurio-studio")
                            .list();
                    ...
                    if (routes.getItems().size() > 0) {
                        ...
                        for (Route route : routes.getItems()) {
                            for (RouteIngress ingress : route.getStatus().getIngress()) {
                                if (ingress.getRouterCanonicalHostname() != null && !ingress.getRouterCanonicalHostname().isEmpty()) {
                        ...
                        if (canonicalHosts.size() == 0) {
                            ...
                            if (canonicalHosts.size() > 1) {
                                log.warn("There are multiple canonical hosts: {}. Selecting randomly.", canonicalHosts);
                            }

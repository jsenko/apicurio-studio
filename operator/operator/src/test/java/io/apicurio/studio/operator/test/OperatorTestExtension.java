...
public class OperatorTestExtension {
    ...
    private void doBeforeAll() {
        Awaitility.setDefaultPollInterval(ofSeconds(2));
        Awaitility.setDefaultTimeout(ofSeconds(WAIT_SECS));

        client = CDI.current().select(KubernetesClient.class).get();
        requireNonNull(client);

        namespace = "apicurio-studio-operator-test-" + UUID.randomUUID().toString().substring(0, 8);
    ...
    public <T extends HasMetadata> void isPresent(Class<T> type, String name) {
        client.resources(type).inNamespace(namespace).withName(name).waitUntilCondition(Objects::nonNull, WAIT_SECS, SECONDS);
    ...
    public <T extends HasMetadata> void isNotPresent(Class<T> type, String name) {
        client.resources(type).inNamespace(namespace).withName(name).waitUntilCondition(Objects::isNull, WAIT_SECS, SECONDS);
    }


    public <T extends HasMetadata> void delete(T resource) {
        client.resource(resource).inNamespace(namespace).delete();
        client.resource(resource).inNamespace(namespace).waitUntilCondition(Objects::isNull, WAIT_SECS, SECONDS);
    }
    ...
    public int portForward(String targetService, int targetPort) {
        ...
        var pf = client.services().inNamespace(namespace).withName(targetService).portForward(targetPort, localPort);
